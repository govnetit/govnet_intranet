...
if ( isset( $wp_version ) && version_compare( $wp_version, '3.5' ) >= 0 ) {
	function erikreart_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
	...
	function erikreart_image_resize( $url, $width = NULL, $height = NULL, $crop = true, $retina = false ) {
		...
		if ( !file_exists( $dest_file_name ) ) {
			...
			elseif ( IMAGETYPE_PNG == $orig_type ) {
				...
				if ( !imagejpeg( $new_image, $dest_file_name, apply_filters( 'resize_jpeg_quality', 80 ) ) )
 ...
 *
 */
add_action( 'delete_attachment', 'erikreart_delete_resized_images' );
function erikreart_delete_resized_images( $post_id ) {
